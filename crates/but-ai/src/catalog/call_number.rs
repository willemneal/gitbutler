//! Call number generation and parsing.
//!
...
impl CallNumberHierarchy {
    ...
    /// Returns call numbers of sibling and cousin nodes.
    pub fn find_neighbors(&self, call_number: &CallNumber) -> Vec<CallNumber> {
        ...
        // Collect all siblings at this level (excluding self).
...
/// `crates/but-ai/src/catalog/classification.rs` -> `ARCH.BUT-AI.CATALOG.CLASSIFICATION`
pub fn call_number_from_path(path: &str, max_depth: usize) -> CallNumber {
    let segments: Vec<String> = path
        .split('/')
        .filter(|s| !s.is_empty())
        // Skip common prefixes that don't carry classification value.
...
/// prefix. Two identical call numbers have distance 0. Two call numbers
/// that share no segments have distance equal to the sum of their depths.
...
mod tests {
    ...
    fn path_to_call_number() {
        let cn = call_number_from_path("crates/but-ai/src/catalog/classification.rs", 5);
        assert_eq!(cn.to_string(), "CRATES.BUT-AI.CATALOG.CLASSIFICATION");
    }
    ...
    fn distance_calculation() {
        ...
        assert_eq!(call_number_distance(&a, &b), 2); // diverge after ARCH.AUTH
    }

    #[test]
    fn max_depth_enforcement() {
        let mut h = CallNumberHierarchy::default_hierarchy(3);
        assert!(!h.register(&CallNumber::parse("A.B.C.D"))); // depth 4 > max 3
        assert!(h.register(&CallNumber::parse("A.B.C"))); // depth 3 == max 3
    }
}
